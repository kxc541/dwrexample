...
<%
	...
	PlayerDTO playerDTO = PlayerServiceDelegator.getPlayer(Integer.parseInt(DataUtil.decrypt(request.getParameter(G4GConstants.FROMID))));

	//MatchDTO matchDTO = TwoPlayerMatchServiceDelegator.getMatchInformation(messageDTO.getMatchid(), request);

	SearchCriteria searchCriteria = new SearchCriteria();
	searchCriteria.setAttribute(G4GConstants.ID, Integer.parseInt(request.getParameter(G4GConstants.MATCHID)));
	...
	searchCriteria.removeAllAttribute();
	searchCriteria.setAttribute(G4GConstants.MATCHID_DB,
			request.getParameter(G4GConstants.MATCHID));
	List<Twoplayermatchcomments> comments = MatchCommentsServiceDelegator
			.getList(searchCriteria);

	String date = DataUtil.getDate(DateUtil.getDateOfTimeZone(DataUtil.getDate(twoplayermatchDTO.getScheduledstartdate().toString(), G4GConstants.DATE_YYYY_MM_DD_HH_MM_SS_sss), userDTO.getOffset()), G4GConstants.DATE_DD_MM_YYYY) ;
	String time = DataUtil.getDate(DateUtil.getDateOfTimeZone(DataUtil.getDate(twoplayermatchDTO.getScheduledstartdate().toString(), G4GConstants.DATE_YYYY_MM_DD_HH_MM_SS_sss), userDTO.getOffset()), G4GConstants.DATE_H_MM_A);
...
<ul class="buttons">
	<li>
		<a
			href="<%=G4GConstants.REJECT_CHALLENGE%><%=twoplayermatchDTO.getId()%><%=G4GConstants.THICKBOX%>"
			class="gray thickbox" onclick="hideNotification();">Reject</a>
	</li>
	<li>
		<a href="<%=G4GConstants.VIEW_CARD%><%=request.getParameter(G4GConstants.MATCHID)%><%=G4GConstants.THICKBOX %>" onclick="hideNotification()" class="green modal thickbox">Details</a>
	</li>
</ul>
