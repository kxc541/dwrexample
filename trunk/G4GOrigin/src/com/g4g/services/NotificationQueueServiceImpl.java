...
/**
 * Class to implement service for NotificationQueues table.
 * 
 * @author Pratik
 * 
 */
public class NotificationQueueServiceImpl implements NotificationQueueService {
	...
	/**
	 * @see com.g4g.services.NotificationQueueService#delete(com.g4g.dto.NotificationqueueDTO)
	 */
	...
	public List<NotificationqueueDTO> getList() throws HibernateException {
		...
		List list = null;
		try {
			subquery.append(SQLConstants.FROM).append(
					SQLConstants.NOTIFICATIONQUEUEDTO_SQL);
			Query query = session.createSQLQuery(subquery.toString());

			// Query query = session.createQuery(" from NotificationqueueDTO");
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						.append(CALLINGMETHOD).append(
								...
								CURRENTMETHOD).append(
								DataUtil.getCurrentMethod()).append(DASHES)
						.append(G4GConstants.NOTIFICATIONQUEUEDTO_LIST_SIZE)
	...
	/**
	 * @see com.g4g.services.NotificationQueueService#getList(com.g4g.dto.SearchListCriteria)
	 */
	@SuppressWarnings(UNCHECKED)
	public List<NotificationqueueDTO> getList(
		...
		AuditUtil
				...
				.writeLog(
						...
						new StringBuffer(this.getClass().getName())
								...
								.append(PARAMETERS)
								.append(
										G4GConstants.NOTIFICATIONQUEUEDTO_LIST_SEARCH_CRITERIA)
								.toString(), Level.INFO);
		Session session = HibernateUtil.getSession();
		Criteria criteria = session.createCriteria(NotificationqueueDTO.class);
		String orderBY = searchListCriteria.getOrderBy();
		if (orderBY != null) {
			if (searchListCriteria.isAsc()) {
				criteria.addOrder(Order.asc(orderBY));
			} else {
				criteria.addOrder(Order.desc(orderBY));
			}
		}
		Set<String> keys = searchListCriteria.getAttributeNames();
		Iterator<String> it = keys.iterator();
		String key = null;
		Object[] value = null;
		while (it.hasNext()) {
			key = it.next();
			value = searchListCriteria.getAttribute(key);
		...
		Transaction transaction = session.beginTransaction();
		List<NotificationqueueDTO> list = new ArrayList<NotificationqueueDTO>();
	...
	/**
	 * @see com.g4g.services.NotificationQueueService#update(com.g4g.dto.NotificationqueueDTO)
	 */
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(
						...
						.append(DataUtil.getCallingMethod()).append(
								...
								G4GConstants.PLAYERIMDTO).toString(),
				Level.INFO);
		Session session = HibernateUtil.getSession();
		Transaction transaction = session.beginTransaction();
		try {
