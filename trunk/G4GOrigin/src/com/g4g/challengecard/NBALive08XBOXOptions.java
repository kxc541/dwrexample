 ...
 *
 */
public class NBALive08XBOXOptions {
	
	public static String getNBALive08XBOX(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(NBALive08XBOXOptions( map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String NBALive08XBOXOptions(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NBALive08XBOX);
					List<GameoptionsDTO> list = map.get(String.valueOf(NBALive08XBOX)+COMMA+G4GConstants.TWO_NUMBER);
					if(list==null) {
					...
					list=map.get(String.valueOf(NBALive08XBOX)+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.THREE_NUMBER);
					...
					searchCriteria.setAttribute(GameIdConstants.optionsequenceid, optionsequenceid + G4GConstants.ONE_NUMBER);
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NBALive08XBOX);
					List<GameoptionsDTO> list = map.get(NBALive08XBOX+COMMA+(optionsequenceid+G4GConstants.ONE_NUMBER));
					if(list==null)
					list =GameOptionServiceDelegator.getList(searchCriteria);
					...
					list=map.get(String.valueOf(NBALive08XBOX)+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.ZERO);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list=map.get(String.valueOf(NBALive08XBOX)+COMMA+optionsequenceid+COMMA+G4GConstants.THREE_NUMBER);
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(NBALive08XBOX, optionsequenceid, searchCriteria);
				}	
			}
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==UnRanked) {
				searchCriteria.removeAllAttribute();
				list=map.get(String.valueOf(NBALive08XBOX)+COMMA+optionsequenceid+COMMA+G4GConstants.ZERO);
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(NBALive08XBOX, optionsequenceid, searchCriteria);
				}	
			}
