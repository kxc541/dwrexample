 ...
 *
 */
public class NBALive08PS2Options {
	
	public static String getNBALive08PS2(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(NBALive08PS2Options( map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String NBALive08PS2Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NBALive08PS2);
					List<GameoptionsDTO> list = map.get(String.valueOf(NBALive08PS2)+COMMA+G4GConstants.TWO_NUMBER);
					if(list==null) {
					...
					list=map.get(String.valueOf(NBALive08PS2)+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.THREE_NUMBER);
					...
					searchCriteria.setAttribute(GameIdConstants.optionsequenceid, optionsequenceid + G4GConstants.ONE_NUMBER);
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NBALive08PS2);
					List<GameoptionsDTO> list = map.get(NBALive08PS2+COMMA+(optionsequenceid+G4GConstants.ONE_NUMBER));
					if(list==null)
					list =GameOptionServiceDelegator.getList(searchCriteria);
					...
					list=map.get(String.valueOf(NBALive08PS2)+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.ZERO);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list=map.get(String.valueOf(NBALive08PS2)+COMMA+optionsequenceid+COMMA+G4GConstants.THREE_NUMBER);
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(NBALive08PS2, optionsequenceid, searchCriteria);
				}	
			}
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==UnRanked) {
				searchCriteria.removeAllAttribute();
				list=map.get(String.valueOf(NBALive08PS2)+COMMA+optionsequenceid+COMMA+G4GConstants.ZERO);
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(NBALive08PS2, optionsequenceid, searchCriteria);
				}	
			}
