 ...
 *
 */
public class FIFA08PS2Options {
	
	public static String getFIFA08PS2(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(FIFA08PS2Options(map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String FIFA08PS2Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, FIFA08PS2);
					List<GameoptionsDTO>list =map.get(FIFA08PS2+COMMA+G4GConstants.TWO_NUMBER); 
					...
					list =map.get(FIFA08PS2+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.FOUR_NUMBER);
					...
					list =map.get(FIFA08PS2+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.ZERO);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =map.get(FIFA08PS2+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.FOUR_NUMBER); 
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(FIFA08PS2, optionsequenceid, searchCriteria);
				}	
			}
