 ...
 * 
 */
public class NHL08PS2Options {

	public static String getNHL08PS2(
		...
		if (optionsequenceid == G4GConstants.ONE_NUMBER) {
			strResponse.append(NHL08PS2Options(map, optionsequenceid,
	...
	public static String NHL08PS2Options(
		...
		if (optionsequenceid == G4GConstants.ONE_NUMBER) {
			...
			if (ChallengeCardGameOptionsServlet.isShootout(valuesequenceid,optionsequenceid)) {
				...
				searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NHL08PS2);
				List<GameoptionsDTO> list = map.get(NHL08PS2+ G4GConstants.COMMA + G4GConstants.ONE_NUMBER+ G4GConstants.COMMA + G4GConstants.ONE_NUMBER);
				if (list == null) {
				...
				list = map.get(NHL08PS2 + G4GConstants.COMMA+ G4GConstants.TWO_NUMBER + G4GConstants.COMMA+ G4GConstants.ZERO);
				...
				searchCriteria.setAttribute(GameIdConstants.optionsequenceid,G4GConstants.TWO_NUMBER);
				searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NHL08PS2);
				List<GameoptionsDTO> list = map.get(NHL08PS2+ G4GConstants.COMMA + G4GConstants.TWO_NUMBER);
				if (list == null) {
					list = GameOptionServiceDelegator.getList(searchCriteria);
				}
				StringBuffer options = new StringBuffer();
				...
				list = map.get(NHL08PS2 + G4GConstants.COMMA
						+ G4GConstants.FOUR_NUMBER);
				if (list == null) {
					list = GameOptionServiceDelegator.getGameOptions(NHL08PS2,
		...
		if (optionsequenceid == G4GConstants.TWO_NUMBER) {
			...
			if (ChallengeCardGameOptionsServlet.isNHLRanked(valuesequenceid,
				...
				searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NHL08PS2);
				List<GameoptionsDTO> list = map.get(NHL08PS2
		...
		if (optionsequenceid == G4GConstants.THREE_NUMBER) {
			...
			if (list != null && list.size() > G4GConstants.ZERO) {
				...
				options.append(ResourceBundleUtil.getValue(option,
						G4GConstants.BLANK, G4GConstants.NOT_REQUIRED));
			}
