 ...
 *
 */
public class NCAAMarchMadness08PS2Options {
	
	public static String getNCAAMarchMadness08PS2(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(NCAAMarchMadness08PS2Options( map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String NCAAMarchMadness08PS2Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, NCAAMarchMadness08PS2);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =GameOptionServiceDelegator.getGameOptions(NCAAMarchMadness08PS2, optionsequenceid, searchCriteria);	
			}
