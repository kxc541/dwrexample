 ...
 *
 */
public class Halo3XBOXOptions {
	
	
	public static String getHalo3XBOX(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(Halo3XBOXOptions(map ,optionsequenceid , valuesequenceidinteger  ));
	...
	public static String Halo3XBOXOptions(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length == G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, Halo3XBOX);
					List<GameoptionsDTO> list = map.get(Halo3XBOX+COMMA+G4GConstants.TWO_NUMBER);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =GameOptionServiceDelegator.getGameOptions(Halo3XBOX, optionsequenceid, searchCriteria);	
			}
