 ...
 * 
 */
public class NCAAFootball08PS2Options {

	public static String getNCAAFootball08PS2(
		...
		if (optionsequenceid == G4GConstants.ONE_NUMBER) {
			strResponse.append(NCAAFootball08PS2Options(map, optionsequenceid,
	...
	public static String NCAAFootball08PS2Options(
		...
		if (optionsequenceid == G4GConstants.ONE_NUMBER) {
			...
			if (valuesequenceidarray.length == G4GConstants.ONE_NUMBER) {
				if (ChallengeCardGameOptionsServlet.isRanked(valuesequenceid,
						optionsequenceid)) {
					SearchCriteria searchCriteria = new SearchCriteria();
					searchCriteria.setAttribute(
							GameIdConstants.valuesequenceid, valuesequenceid);
					searchCriteria.setAttribute(
							GameIdConstants.optionsequenceid, optionsequenceid
									+ G4GConstants.ONE_NUMBER);
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB,
							NCAAFootball08PS2);
					List<GameoptionsDTO> list = map.get(String.valueOf(NCAAFootball08PS2)+ G4GConstants.COMMA+ String.valueOf(optionsequenceid + G4GConstants.ONE_NUMBER));
					if (list == null) {
					...
					list = map.get(String.valueOf(NCAAFootball08PS2)+ G4GConstants.COMMA + (G4GConstants.THREE_NUMBER));
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB,NCAAFootball08PS2);
		...
		if (optionsequenceid == G4GConstants.TWO_NUMBER) {
			...
			if (valuesequenceidarray[G4GConstants.ONE_NUMBER] == UnRanked) {
				list = GameOptionServiceDelegator.getGameOptions(
						NCAAFootball08PS2, optionsequenceid, searchCriteria);
			}
			StringBuffer options = new StringBuffer();
