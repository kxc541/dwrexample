 ...
 *
 */
public class TigerWoods08PS3Options {
	
	public static String getTigerWoods08PS3(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(TigerWoods08PS3Options( map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String TigerWoods08PS3Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, TigerWoods08PS3);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =GameOptionServiceDelegator.getGameOptions(TigerWoods08PS3, optionsequenceid, searchCriteria);	
			}
