 ...
 *
 */
public class Madden08PS2Options {
	
	
	public static String getMadden08PS2(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(Madden08PS2Options( map ,optionsequenceid , valuesequenceidinteger  ));
	...
	public static String Madden08PS2Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length == G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, Madden08PS2);
					List<GameoptionsDTO> list = map.get(String.valueOf(Madden08PS2)+COMMA+String.valueOf(G4GConstants.TWO_NUMBER));
					if(list==null) {
					...
					re.append(ResourceBundleUtil.getValue(GameIdConstants.options,ResourceBundleUtil.getValue(option, String.valueOf(dto.getOptionsequenceid()),dto.getValue())));
					 list = map.get(String.valueOf(Madden08PS2)+COMMA+String.valueOf(G4GConstants.THREE_NUMBER));
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				searchCriteria.setAttribute(valueid, G4GConstants.ONE_NUMBER);
				 list = map.get(String.valueOf(Madden08PS2)+COMMA+String.valueOf(optionsequenceid+G4GConstants.ONE_NUMBER));
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(Madden08PS2, optionsequenceid, searchCriteria);
				}	
			}
