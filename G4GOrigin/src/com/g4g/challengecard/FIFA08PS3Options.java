 ...
 *
 */
public class FIFA08PS3Options {
	
	public static String getFIFA08PS3(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(FIFA08PS3Options(map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String FIFA08PS3Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, FIFA08PS3);
					List<GameoptionsDTO>list =map.get(FIFA08PS3+COMMA+G4GConstants.TWO_NUMBER); 
					...
					list =map.get(FIFA08PS3+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.FOUR_NUMBER);
					...
					list =map.get(FIFA08PS3+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.ZERO);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =map.get(FIFA08PS3+COMMA+G4GConstants.TWO_NUMBER+COMMA+G4GConstants.FOUR_NUMBER); 
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(FIFA08PS3, optionsequenceid, searchCriteria);
				}	
			}
