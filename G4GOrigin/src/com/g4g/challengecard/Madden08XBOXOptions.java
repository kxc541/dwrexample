 ...
 *
 */
public class Madden08XBOXOptions {
	
	
	public static String getMadden08XBOX(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(Madden08XBOXOptions( map ,optionsequenceid , valuesequenceidinteger  ));
	...
	public static String Madden08XBOXOptions(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length == G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, Madden08XBOX);
					List<GameoptionsDTO> list = map.get(String.valueOf(Madden08XBOX)+COMMA+String.valueOf(optionsequenceid+G4GConstants.ONE_NUMBER));
					if(list==null) {
					...
					re.append(ResourceBundleUtil.getValue(GameIdConstants.options,ResourceBundleUtil.getValue(option, String.valueOf(dto.getOptionsequenceid()),dto.getValue())));
					 list = map.get(String.valueOf(Madden08XBOX)+COMMA+String.valueOf(G4GConstants.THREE_NUMBER));
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				searchCriteria.setAttribute(valueid, G4GConstants.ONE_NUMBER);
				 list = map.get(String.valueOf(Madden08XBOX)+COMMA+String.valueOf(optionsequenceid+G4GConstants.ONE_NUMBER));
				if(list==null) {
					list =GameOptionServiceDelegator.getGameOptions(Madden08XBOX, optionsequenceid, searchCriteria);
				}	
			}
