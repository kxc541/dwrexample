 ...
 *
 */
public class MLB08theShowPS2Options {
	
	public static String getMLB08theShowPS2(THashMap<String, List<GameoptionsDTO>> map ,Integer[] valuesequenceidinteger, int gameid , int optionsequenceid) {
		StringBuffer strResponse = new StringBuffer();
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			strResponse.append(MLB08theShowPS2Options(map ,optionsequenceid , valuesequenceidinteger ));
	...
	public static String MLB08theShowPS2Options(THashMap<String, List<GameoptionsDTO>> map ,int  optionsequenceid , Integer... valuesequenceidarray) {
		if(optionsequenceid==G4GConstants.ONE_NUMBER) {
			...
			if(valuesequenceidarray.length ==  G4GConstants.ONE_NUMBER) {
				if(ChallengeCardGameOptionsServlet.isRanked(valuesequenceid, optionsequenceid)) {
					...
					searchCriteria.setAttribute(G4GConstants.GAMEID_DB, MLB08theShowPS2);
			...
			if(valuesequenceidarray[G4GConstants.ONE_NUMBER]==Ranked) {
				...
				list =GameOptionServiceDelegator.getGameOptions(MLB08theShowPS2, optionsequenceid, searchCriteria);	
			}
