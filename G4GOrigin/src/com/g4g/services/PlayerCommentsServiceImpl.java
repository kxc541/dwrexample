...
import static com.g4g.utils.G4GConstants.PLAYER_COMMENTS_LIST_SIZE;
import static com.g4g.utils.G4GConstants.PLAYER_COMMENTS_LIST_SIZE_CRITERIA;
import static com.g4g.utils.G4GConstants.PLAYERCOMMENTSDTO;
...
/**
 * Class to implement service for PlayerComments table.
 * 
 * @author Pratik
 * 
 */

public class PlayerCommentsServiceImpl implements PlayerCommnetsService {
	 ...
	 */
	public PlayercommentsDTO add(PlayercommentsDTO dto)
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						...
						.append(PLAYERCOMMENTSDTO).toString(), Level.INFO);
	...
	/**
	 * @see com.g4g.services.PlayerCommnetsService#delete(com.g4g.dto.PlayercommentsDTO)
	 */
	...
	public List<PlayercommentsDTO> getList() throws HibernateException {
		...
		Session session = HibernateUtil.getSession();
		List<PlayercommentsDTO> list = null;
		try {
			Query query = session.createQuery(new StringBuffer(
					SQLConstants.FROM).append(SQLConstants.PLAYERCOMMENTSDTO_SQL)
					.toString());
			Transaction transaction = session.beginTransaction();
			list = query.list();
			transaction.commit();
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						.append(CALLINGMETHOD).append(
								...
								CURRENTMETHOD).append(
								DataUtil.getCurrentMethod()).append(DASHES)
						.append(PLAYER_COMMENTS_LIST_SIZE).append(list.size())
						.toString(), Level.INFO);
		AuditUtil.getInstance().writeLog(
				AuditUtil.FILE_TYPE_G4G,
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
	...
	/**
	 * @see com.g4g.services.PlayerCommnetsService#getList(com.g4g.dto.SearchCriteria)
	 */
	@SuppressWarnings(UNCHECKED)
	public List<PlayercommentsDTO> getList(SearchCriteria searchCriteria)
			throws HibernateException {
		AuditUtil.getInstance().writeLog(
				AuditUtil.FILE_TYPE_G4G,
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
		...
		Session session = HibernateUtil.getSession();
		Criteria criteria = session.createCriteria(PlayercommentsDTO.class);
		...
		Transaction transaction = session.beginTransaction();
		List<PlayercommentsDTO> list = new ArrayList<PlayercommentsDTO>();
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						.append(CALLINGMETHOD).append(
								...
								CURRENTMETHOD).append(
								DataUtil.getCurrentMethod()).append(DASHES)
						.append(PLAYER_COMMENTS_LIST_SIZE_CRITERIA).append(
	...
	/**
	 * @see com.g4g.services.PlayerCommnetsService#update(com.g4g.dto.PlayercommentsDTO)
	 */
	public PlayercommentsDTO update(PlayercommentsDTO dto)
			throws HibernateException {
		AuditUtil.getInstance().writeLog(
				AuditUtil.FILE_TYPE_G4G,
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
