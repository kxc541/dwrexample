...
import static com.g4g.utils.G4GConstants.PLAYER_NETWORK_DTO;
import static com.g4g.utils.G4GConstants.PLAYER_NETWORK_LIST_SIZE;
import static com.g4g.utils.G4GConstants.PLAYER_NETWORK_LIST_SIZE_CRITERIA;
...
/**
 * Class to implement service functions for Playernetwork table.
 * 
 * @author Ankur
 * 
 */
public class PlayerNetworkServiceImpl implements PlayerNetworkService {
	...
	public PlayernetworkDTO add(PlayernetworkDTO dto) throws HibernateException {
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						...
						.append(PLAYER_NETWORK_DTO).toString(), Level.INFO);
	...
	public List<PlayernetworkDTO> getList() throws HibernateException {
		...
		Session session = HibernateUtil.getSession();
		List<PlayernetworkDTO> list = null;
		try {
			Query query = session.createQuery(new StringBuffer(
					SQLConstants.FROM).append(SQLConstants.PLAYERNETWORKDTO_SQL)
					.toString());
			Transaction transaction = session.beginTransaction();
			list = query.list();
			transaction.commit();
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						.append(CALLINGMETHOD).append(
								...
								CURRENTMETHOD).append(
								DataUtil.getCurrentMethod()).append(DASHES)
						.append(PLAYER_NETWORK_LIST_SIZE).append(list.size())
						.toString(), Level.INFO);
		AuditUtil.getInstance().writeLog(
				AuditUtil.FILE_TYPE_G4G,
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
	 ...
	 */
	@SuppressWarnings(UNCHECKED)
	public List<PlayernetworkDTO> getList(SearchCriteria searchCriteria)
		...
		Session session = HibernateUtil.getSession();
		Criteria criteria = session.createCriteria(PlayernetworkDTO.class);
		...
		Transaction transaction = session.beginTransaction();
		List<PlayernetworkDTO> list = new ArrayList<PlayernetworkDTO>();
		...
		AuditUtil.getInstance().writeLog(
				...
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
						.append(CALLINGMETHOD).append(
								...
								CURRENTMETHOD).append(
								DataUtil.getCurrentMethod()).append(DASHES)
						.append(PLAYER_NETWORK_LIST_SIZE_CRITERIA).append(
	 ...
	 */
	public PlayernetworkDTO update(PlayernetworkDTO dto)
			throws HibernateException {
		AuditUtil.getInstance().writeLog(
				AuditUtil.FILE_TYPE_G4G,
				new StringBuffer(this.getClass().getName()).append(COLON_WITH_SPACES)
	...
	/**
	 * @see PlayerNetworkService#getGame(int)
	 */
